...
class TestCase(MyTestOn):
    ...
    def test_01(self):
        ...
        for i in range(range_num):
            # 下限价委托
            buy_order_resp = self.session.post(url=base+sda_order_create_url, data=get_sda_order_create_param(sda_id=sda_id,
                                                                                                              order_type=多单,
                                                                                                              order_price_type=限价,
                                                                                                              lever=lever,
        ...
        sell_order_resp = self.session_54.post(url=base+sda_order_create_url, data=get_sda_order_create_param(sda_id=sda_id,
                                                                                                              order_type=空单,
                                                                                                              order_price_type=限价,
                                                                                                              lever=lever,
                                                                                                              order_price=deal_price,
        ...
        flag = assert_one(int(before_balance), int(after_balance) + int(sell_num * deal_price / lever / 100000000))
    ...
    def test_02(self):
        ...
        # 下委托后查询可用余额
        # time.sleep(3)
        after_info_dict = account_info_sync(sync_id=sync_id, user=self.seller, session=self.sell_session, sda_id=sda_id, price=1000000000)
        after_balance = after_info_dict["balance"]

        flag = assert_one(int(before_balance), int(after_balance) + buy_num * price / lever / 100000000)
        self.assertTrue(flag)
