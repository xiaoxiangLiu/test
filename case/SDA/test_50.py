...
class TestCase(MyTestOn):
    ...
    def test_01(self):
        ...
        多单持仓，平仓，53用户下多单与之成交。
        """
        logger.info("用例编号：50-1---多单持仓，限价平仓，53用户下限价多单与之成交，验证持仓数据")
        range_num = 10
        price = 120*100000000
        ...
        sync_id = JMESPathExtractor().extract(query="syncLockKey", body=buy_53_resp.text)
        logger.info("接口：{0}---返回信息：{1}".format(sda_order_create_url, buy_53_resp.json()))

        # time.sleep(0.3)

        after_53_info_dict = account_info_sync(sync_id=sync_id, user=self.user_53, session=self.session_53,
                                               sda_id=sda_id)
        ...
        flag_51 = assert_one(int(after_account_balance), 10000217795969078)
    ...
    def test_02(self):
        ...
        空单持仓，平空与空单成交
        """
        logger.info("用例编号：50-2---空单持仓，平空与空单成交")
        range_num = 10
        price = 120*100000000
        buy_num = 100*100000000
        ...
        sync_id_53 = JMESPathExtractor().extract(query="syncLockKey", body=buy_53_resp.text)
        logger.info("接口：{0}---返回信息：{1}".format(sda_order_create_url, buy_53_resp.json()))

        # time.sleep(3)

        after_53_info_dict = account_info_sync(sync_id=sync_id_53, user=self.user_53, session=self.session_53,
                                               sda_id=sda_id)
        after_53_account_balance = after_53_info_dict["balance"]
    ...
    def test_03(self):
        ...
        多单持仓，平仓，53用户下多单与之成交。
        """
        logger.info("用例编号：50-3---多单持仓，限价平仓，53用户下市价多单与之成交，验证持仓数据")
        range_num = 10
        price = 120*100000000
        buy_num = 100*100000000
