...
class TestCase(MyTestTwoUser):
    ...
    def test_01(self):
        """
        限价多单1单，市价空单1单，多单完全成交，空单部分成交，验证合约状态、用户余额
        """
        logger.info("用例编号：44-1---限价多单1单，市价空单1单，多单完全成交，空单部分成交，验证合约状态、用户余额")
        sell_num = 100000000
        buy_num = 200000000
        ...
        # 检验持仓均价，持仓量，保证金
        sell_flag = assert_one(int(self.sda_balance - sell_num * deal_price / 100000000), int(after_sell_balance))
        buy_flag = assert_one(int(self.sda_balance - sell_num * deal_price / 100000000), int(after_buy_balance))
        self.assertListEqual([True, True], [sell_flag, buy_flag])

    def test_03(self):
        """
        限价多单1单，市价空单1单，多单部分成交，空单完全成交，验证合约状态、用户余额
        """
        logger.info("用例编号：44-3---限价多单1单，市价空单1单，多单部分成交，空单完全成交，验证合约状态、用户余额")
        sell_num = 100000000
        buy_num = 200000000
        ...
        # 检验持仓均价，持仓量，保证金
        sell_flag = assert_one(int(self.sda_balance - sell_num * deal_price / 100000000), int(after_sell_balance))
        buy_flag = assert_one(int(self.sda_balance - buy_num * deal_price / 100000000), int(after_buy_balance))
        self.assertListEqual([True, True], [sell_flag, buy_flag])
