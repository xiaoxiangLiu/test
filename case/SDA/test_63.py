...
class TestCase(MyTestOnTwo):
    ...
    def test_01(self):
        ...
        buy_resp = self.session.post(url=base+sda_order_create_url, data=get_sda_order_create_param(
            sda_id=sda_id, order_type=平多, order_price_type=市价, order_price=price, order_num=0
        ))
    ...
    def test_02(self):
        ...
        """
        logger.info("用例编号：63-2---空单持仓中，部分市价平仓，对应委托成交，部分成交，未成交部分自动撤单，验证用户余额，委托状态，手续费")
        range_num = 10
        price = 20*100000000
        buy_num = 2*100000000
        sell_num = 5*100000000
        ...
        flag_51 = assert_one(int(after_account_balance), 9999985080406848)
