...
class TestCase(MyTestTwoUser):
    ...
    def test_01(self):
        """
        限价多单1单，市价空单1单，完全成交，验证合约状态、用户余额
        """
        logger.info("用例编号：43-1---限价多单1单，市价空单1单，完全成交，验证合约状态、用户余额")
        deal_num = 100000000
        ...
        sell_resp = self.sell_session.post(url=base+sda_order_create_url, data=get_sda_order_create_param(
            sda_id=sda_id, order_type=空单,order_price_type=市价,order_price=deal_price,order_num=deal_num
        ))
    ...
    def test_02(self):
        ...
        """
        logger.info("用例编号：43-2---限价空单1单，市价多单1单，完全成交，验证合约状态、用户余额")
        deal_num = 100000000
        ...
        # time.sleep(2)
        sell_resp = self.sell_session.post(url=base+sda_order_create_url, data=get_sda_order_create_param(
            sda_id=sda_id, order_type=多单,order_price_type=市价,order_price=deal_price,order_num=deal_num
        ))
        sell_sync_id = JMESPathExtractor().extract(query="syncLockKey", body=sell_resp.text)
        logger.info("用户：{0}--接口：{1}--状态：{2}--类型：空单--返回信息：{3}".format(
            self.seller, sda_order_create_url, sell_resp.status_code, sell_resp.json()
