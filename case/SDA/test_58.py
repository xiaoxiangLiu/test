...
class TestCase(MyTestOn):
    ...
    def test_01(self):
        ...
        """
        logger.info("用例编号：58-1----多单持仓，市价全平，多个对应委托，成交其中部分委托，验证委托状态、用户余额、手续费")
        ...
        buy_resp = self.session.post(url=base+sda_order_create_url, data=get_sda_order_create_param(
            sda_id=sda_id, order_type=平多, order_price_type=市价, order_num=0
        ))
    ...
    def test_02(self):
        ...
        sell_resp = self.sell_session.post(url=base+sda_order_create_url, data=get_sda_order_create_param(
            sda_id=sda_id, order_type=平空, order_price_type=市价, order_num=0
        ))
        ...
        flag_53 = assert_one(self.sda_balance - int(price * sell_num * range_num / 100000000), int(after_53_account_balance))
        self.assertListEqual([True, True], [flag_51, flag_53])
