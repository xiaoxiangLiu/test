...
class TestCase(MyTestOn):
    ...
    def test_02(self):
        ...
        空单持仓，平空与空单成交
        """
        logger.info("用例编号：90-2---空单持仓，部分平仓，追加仓位，全平")
        range_num = 10
        price = 120*100000000
        buy_num = 120*100000000
        first_num = 10*100000000
        add_first_num = 30*100000000
        add_first_price = 120*100000000
        ...
        second_buy_sync_id = JMESPathExtractor().extract(query="syncLockKey", body=second_sell_resp.text)
        logger.info("接口：{0}---返回信息：{1}".format(sda_order_create_url, second_sell_resp.json()))

        # time.sleep(3)
        after_53_info_dict = account_info_sync(user=self.user_53, session=self.session_53, sda_id=sda_id,
                                               sync_id=second_buy_sync_id)
        after_53_account_balance = after_53_info_dict["balance"]

        after_info_dict = account_info_sync(sync_id=second_buy_sync_id, user=self.seller, session=self.sell_session, sda_id=sda_id)
        after_account_balance = after_info_dict["balance"]

        flag_51 = assert_one(int(after_account_balance), 9999771764030922)
        self.assertTrue(flag_51)
