...
class TestCase(MyTestOn):
    ...
    def test_01(self):
        ...
        """
        logger.info("用例编号：52-1---多单持仓中，部分市价平仓，无对应委托成交，自动撤单，验证用户余额、委托状态")
        range_num = 10
        price = 120*100000000
        ...
        # time.sleep(2)
        after_buy_account_dict = account_info_sync(sync_id=sync_id, user=self.buyer, session=self.session, sda_id=sda_id)
    ...
    def test_02(self):
        ...
        """
        logger.info("用例编号：52-2---空单持仓中，部分市价平仓，无对应委托成交，自动撤单，验证用户余额，委托状态")
        range_num = 10
        price = 120*100000000
        buy_num = 50*100000000
